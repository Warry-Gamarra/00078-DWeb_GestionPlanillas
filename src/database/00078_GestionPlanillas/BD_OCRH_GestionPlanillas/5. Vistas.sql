...
FROM 
	...
	dbo.TC_Sexo AS se ON se.I_SexoID = per.I_SexoID LEFT JOIN
...
SELECT
	...
	est.I_EstadoID, est.T_EstadoDesc, vin.I_VinculoID, vin.C_VinculoCod, vin.T_VinculoDesc, reg.I_RegimenID, reg.T_RegimenDesc,
	...
	trabcat.B_Habilitado, trabcat.B_EsJefe
FROM 
	dbo.TC_Persona AS per INNER JOIN
	dbo.TC_Trabajador AS trab ON trab.I_PersonaID = per.I_PersonaID INNER JOIN
